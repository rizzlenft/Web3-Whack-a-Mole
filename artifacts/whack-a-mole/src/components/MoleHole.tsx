...
const DEGEN_PHRASES = [
  ...
  'REKT SER', 'PAPER HANDS', 'DOWN BAD', 'GM NEVER', 'COPE',
];
...
export function MoleHole({ mole, onWhack }: MoleHoleProps) {
  ...
  const phrase  = React.useRef(getRandomPhrase());

  React.useEffect(() => {
    if (!mole.active) phrase.current = getRandomPhrase();
  }, [mole.active]);
    ...
    <div className="relative w-16 h-16 sm:w-24 sm:h-24 md:w-32 md:h-32 mx-auto select-none mt-3">
      ...
      <AnimatePresence>
        {!mole.active && (
          <motion.div
            ...
            exit={{ opacity: 0 }}
            className="absolute bottom-[12%] left-[20%] w-[60%] h-[25%] bg-primary/30 rounded-full blur-md z-0 pointer-events-none"
          />
        )}
      </AnimatePresence>

      {/* ── Mole rising from hole ─────────────────── */}
      <div className="absolute bottom-[18%] left-[8%] w-[84%] h-[115%] overflow-hidden z-10 pointer-events-none">
        ...
        >
          {/* Ground shadow */}
          {visible && (
            <div className="absolute bottom-0 w-3/4 h-3 bg-black/70 rounded-full blur-sm" />
          )}

          {/* PFP circle */}
          <div className={cn(
            "w-[88%] aspect-square rounded-full border-4 overflow-hidden bg-zinc-950 relative pointer-events-auto transition-all",
            visible ? "border-primary neon-border-pulse shadow-[0_0_24px_rgba(255,0,255,0.9)]" : "border-zinc-800",
            mole.whacked ? "brightness-40 grayscale border-zinc-800 shadow-none" : ""
          )}>
            <img
              src={pfpSrc}
              alt="Scammer PFP"
            ...
            <AnimatePresence>
              {mole.whacked && (
                <motion.div
                  ...
                  className="absolute inset-0 flex items-center justify-center z-30 pointer-events-none text-3xl md:text-4xl"
                >
                  💥
                </motion.div>
              )}
      ...
      <AnimatePresence>
        {mole.whacked && (
          <motion.div
            initial={{ scale: 0.2, opacity: 0, y: 10, rotate: -25 }}
            animate={{ scale: 1.3, opacity: 1, y: -50, rotate: 8 }}
            exit={{ scale: 1.8, opacity: 0, y: -90 }}
            transition={{ type: 'spring', duration: 0.5 }}
            className="absolute top-[-10%] left-[-15%] w-[130%] flex items-center justify-center z-40 pointer-events-none"
          >
            <span
              className="font-display text-sm md:text-xl text-yellow-300 uppercase tracking-tight text-center leading-none drop-shadow-[0_3px_6px_rgba(0,0,0,0.9)]"
              style={{ textShadow: '2px 2px 0 #000, 0 0 10px hsl(315,100%,60%)' }}
            >
              {phrase.current}
            </span>
          </motion.div>
